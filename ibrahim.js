...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `KIBORE MD BOT is online!\n${currentDateTime}`; // Format the bio text
...
// Other functions (auto-react, anti-delete, etc.) as needed
  ...
  if (conf.ANTICALL === 'no') {
    ...
    await zk.sendMessage(callerId, {
      text: "Am Kibore Md Bot,, My owner is unavailable try again later"
    });
  }
});
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*KIBORE_MD WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ KIBORE_MD is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ KIBORE_MD Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("KIBORE_MD is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading KIBORE_MD Commands ...\n");
